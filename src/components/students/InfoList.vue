...
<script>
import { infoApi, infoAddApi, infoEditApi, infoDelApi } from "@/api/api";
...
export default {
  ...
  methods: {
    ...
    edit(row) {
      ...
      this.form = { ...row }; // 不直接绑定row，防止修改时表格数据即时同步修改
    },
    del(row) {
      this.$alert("确认删除吗", "提示", {
        confirmButtonText: "确定",
        callback: () => {
          infoDelApi(row.id).then((res) => {
            if (res.data.status === 200) {
              this.getData();
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          if (this.state) {
            ...
            infoEditApi(this.form).then((res) => {
              if (res.data.status === 200) {
                this.getData();
                this.dialogFormVisible = false;
                this.$message({ message: res.data.message, type: "success" });
